...
class LaunchPagingAdapter : PagingDataAdapter<LaunchEntity, LaunchPagingAdapter.LaunchViewHolder>(DiffCallback) {
    ...
    class LaunchViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(launch: LaunchEntity?) {
            launch?.let {
                ...
                dateTextView.text = formatDate(it.dateUtc)
                flightNumberTextView.text = "Flight #${it.flightNumber}"
                rocketTypeTextView.text = it.rocketType ?: "Unknown Rocket"
    ...
    companion object {
        private val DiffCallback = object : DiffUtil.ItemCallback<LaunchEntity>() {
